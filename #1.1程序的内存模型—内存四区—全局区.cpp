...
int main()
{
	//全局区：全局变量和静态变量存放在此，全局区还包含了常量区，字符串常量和其他常量（const修饰的变量）也存放在此 
	...
	cout <<"全局常量c_g_a的地址未："<< (long long)&c_g_a << endl; 
	cout <<"全局常量c_g_b的地址未："<< (long long)&c_g_b << endl; 
	
	const int c_l_a = 10;// c-const g-global l-local
	const int c_l_b = 10;
	
	cout <<"局部常量从c_l_a的地址为："<< (long long)&c_l_a << endl;
	cout <<"局部常量从c_l_b的地址为："<< (long long)&c_l_b << endl;
